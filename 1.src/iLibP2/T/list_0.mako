...
<%block name="head_script">
    ...
    <script >
        ...
        function table_row_del(e, value, row, index){
            $.post("/${fn}/ajax",{cmd:"del",id:value},function(r){ refresh();},"json")
        }
        //初始化用于编辑数据的vm
        function init_vm(){vm= new Vue({el:'#rowData',data:{row: {
            %for rec in recs:
                ${rec.colname}   : '',
            %endfor
            the_id  : '',
        ...
        //初始化用于搜索的参数
        function init_vms(){vm_s= new Vue({el:'#searchform',data:{row: {
            %for rec in recs:
                ${rec.colname}   : '',
            %endfor
            cmd     : 'ajax'
...
<!--数据主体显示-->
<table id="the_data" style="margin-bottom: 15px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;">
    <thead>
    <tr>
        %for rec in recs:
        <th data-field="${rec.colname}"          data-title="${rec.colcomment}"></th>
        %endfor
        <th data-title="操作" data-events='table_events'  data-formatter='table_formatter' ></th>
    </tr>
    </thead>
</table>
<%text>
...
<%block name ="include_module">
    ...
    <%text><%include </%text>file="${fn}_1.html"/>
    <%text><%include </%text>file="${fn}_2.html"/>
    <%text><%include </%text>file="${fn}_3.html"/>
<%text>
</%block>
</%text>
